...
export class Mainbanner extends View {
    ...
    template() {
        const {banner, sidebar, selected} = this.store.state;
        return ` ${banner
            .map(
                (img, idx) =>
                    `<img class="main_bg ${
    ...
    changeBanner = ( next) => {
        const spans = this.#spans(), imgs = this.#images()
    ...
    setEvent() {
        ...
        this.addEvent(
            ...
            "span",
            (e) => {
                this.throttle(() => this.changeBanner(parseInt(e.target.dataset.idx)), 100);
                this.startAuto(auto, 3000);
            },
            100
        );
